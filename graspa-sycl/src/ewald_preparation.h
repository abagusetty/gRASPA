...
{
  printf(" ****** CHECKING StructureFactors (SF) Stored on CPU vs. GPU ****** \n");
  size_t numberOfStructureFactors = (Box.kmax.x() + 1) * (2 * Box.kmax.y() + 1) * (2 * Box.kmax.z() + 1);
  Complex GPUSF[numberOfStructureFactors];
  sycl_get_queue()->memcpy(GPUSF, Box.AdsorbateEik, numberOfStructureFactors * sizeof(Complex)).wait();

  printf("CPU SF: %zu, GPU SF: %zu\n", SystemComponents.AdsorbateEik.size(), numberOfStructureFactors);

  size_t numSFCPU            = SystemComponents.AdsorbateEik.size();
  if(numberOfStructureFactors != numSFCPU) printf("ERROR: Number of CPU StructureFactors does NOT EQUAL to the GPU one!!!");
  size_t counter = 0;
  for(size_t i = 0; i < numberOfStructureFactors; i++)
  {
    ...
    if(i < 10)
      printf("StructureFactor %zu, CPU: %.5f %.5f, GPU: %.5f %.5f\n", i, SystemComponents.AdsorbateEik[i].real(), SystemComponents.AdsorbateEik[i].imag(), GPUSF[i].real, GPUSF[i].imag);
    if(diff_real > 1e-10 || diff_imag > 1e-10)
    {
      counter++;
      if(counter < 10) printf("There is a difference in GPU/CPU StructureFactor at position %zu: CPU: %.5f %.5f, GPU: %.5f %.5f\n", i, SystemComponents.AdsorbateEik[i].real(), SystemComponents.AdsorbateEik[i].imag(), GPUSF[i].real, GPUSF[i].imag);
    }
  }
  if(counter >= 10) printf("More than 10 StructureFactors mismatch.\n");
  //Also check Framework Eik vectors//
  printf(" ****** CHECKING Framework StructureFactors Stored on CPU ****** \n");
  for(size_t i = 0; i < numberOfStructureFactors; i++)
  {
    if(i < 10) printf("Framework Structure Factor %zu, real: %.5f imag: %.5f\n", i, SystemComponents.FrameworkEik[i].real(), SystemComponents.FrameworkEik[i].imag());
  }
}
